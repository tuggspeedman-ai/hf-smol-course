...
- **The default VLM fine-tuning recipe doesn't touch the vision encoder.** I came into U3 without much sense of how vision-language models are built. The shape is: a vision encoder (SigLIP for SmolVLM2) turns the image into visual tokens, a small projector aligns those with the LLM's embedding space, and the LLM treats them like any other tokens. The common default for fine-tuning is to freeze the vision encoder and LoRA only the LLM. The goal isn't to improve the model's vision, it's to teach the LLM how to use the features it's already getting. My ChartQA adapter is exactly that: same eyes, new way of talking about what they see.
